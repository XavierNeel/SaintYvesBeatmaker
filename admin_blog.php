...
<?php foreach ($list_title as $row) { ?>
    <option value="<?php echo @$row->id_blog; ?>" <?php if (@$row->id_blog == @$_POST["id_blog"]) {echo " selected";} ?>><?php echo $row->title_blog; ?></option><?php } ?>
</select>

<!-- LISTE CATEGORIES -->
<label for="">Choix des categories*</label><br>
<select name="id_blog_cat" id="id_blog_cat" class="form-control" required>
<option value="">Choix de votre catégorie</option><?php foreach ($list_blog_cat as $row) { ?><option value="<?php echo $row->id_blog_cat; ?>" <?php if ($row->id_blog_cat == @$blog_unique->id_blog_cat) {echo " selected";} ?>><?php echo $row->name_blog_cat; ?></option> <?php } ?>
</select>
